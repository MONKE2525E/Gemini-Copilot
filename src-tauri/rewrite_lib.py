...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            let resource_path = app.path().resource_dir().unwrap_or_default().join("python-sidecar").join("main.py");
            let sidecar_path = if resource_path.exists() { resource_path } else {
                let current = std::env::current_dir().unwrap();
                if current.ends_with("src-tauri") { current.parent().unwrap().join("python-sidecar").join("main.py") }
                else { current.join("python-sidecar").join("main.py") }
            };
            ...
            let mut child = Command::new(python_to_use)
                .arg(&sidecar_path).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
                .spawn().expect("Failed to spawn Python sidecar");
...
with io.open("G:/Gemini Desktop/gemini-copilot/src-tauri/src/lib.rs", "w", encoding="utf-8") as f:
    f.write(rust_code)
